...
class TaskViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // -----------------------------------------------------------------------------------------
      ...
      data class DescendantNode(val task: TaskItem, val depthLevel: Int)
...
suspend fun getAllDescendantsTree(taskId: Long): List<DescendantNode> = withContext(Dispatchers.IO) {
    val result = mutableListOf<DescendantNode>()
...
suspend fun getAllPotentialParents(excludeTaskId: Long, isCopy: Boolean = false): List<TaskItem> = withContext(Dispatchers.IO) {
    val all = dao.getAllTasksSnapshot()
    if (isCopy) {
        all
    } else {
        val descendants = getAllDescendantsTree(excludeTaskId).map { it.task.id }.toSet()
...
fun executeAdvancedCopy(
    ...
    viewModelScope.launch(Dispatchers.IO) {
        repeat(numberOfCopies) {
            for (targetParentId in targetParentIds) {
                deepCopySelective(task, targetParentId, copyAllSubtasks, selectedSubtaskIds)
            }
        }
    }
}

private suspend fun deepCopySelective(
    task: TaskItem,
    newParentId: Long?,
    copyAll: Boolean,
    selectedSubtaskIds: Set<Long>
    ...
    val siblings = dao.getSubtasksSnapshot(newParentId)
    val copyTitle = if (newParentId == task.parentId) "${task.title} (Copy)" else task.title
    val now = System.currentTimeMillis()
    val copy = task.copy(
        id = 0L,
        parentId = newParentId,
    ...
    val checklists = dao.getChecklistSnapshot(task.id)
    for (item in checklists) dao.insertChecklistItem(item.copy(id = 0L, taskId = newId))

    val attachments = dao.getAttachmentsSnapshot(task.id)
    for (att in attachments) dao.insertAttachment(att.copy(id = 0L, taskId = newId))

    val children = dao.getSubtasksSnapshot(task.id)
    for (child in children) {
        if (copyAll || child.id in selectedSubtaskIds) {
            deepCopySelective(child, newId, copyAll, selectedSubtaskIds)
        }
    }
}
...
fun executeAdvancedMove(
    ...
    viewModelScope.launch(Dispatchers.IO) {
        val targetParentId = targetParentIds.firstOrNull() ?: return@launch
        val now = System.currentTimeMillis()
        ...
        if (!moveAllSubtasks) {
            ...
            for (child in children) {
                if (child.id !in selectedSubtaskIds) {
                    val oldParentSiblings = dao.getSubtasksSnapshot(task.parentId)
                    dao.updateTask(child.copy(parentId = task.parentId, orderIndex = oldParentSiblings.size, lastModifiedTimestamp = now))
                }
            }
        }
    }
...
fun deleteAttachmentsBatch(attachments: List<RichAttachment>) {
    viewModelScope.launch(Dispatchers.IO) {
        for (att in attachments) {
            dao.deleteAttachment(att)
            touchTask(att.taskId)
        }
    }
}

fun reverseTaskRoles(taskA: TaskItem, taskC: TaskItem, nestUnder: Boolean, onComplete: () -> Unit = {}) {
    viewModelScope.launch(Dispatchers.IO) {
        val now = System.currentTimeMillis()
        if (nestUnder) {
            ...
            syncTaskToCalendar(updatedC)
            syncTaskToCalendar(updatedA)
        } else {
            val updatedC = taskC.copy(parentId = taskA.parentId, orderIndex = taskA.orderIndex, lastModifiedTimestamp = now)
...
fun executeCustomHierarchyTransfer(plans: List<TaskHierarchyPlan>, isCopy: Boolean, numberOfCopies: Int = 1, onComplete: () -> Unit = {}) {
    viewModelScope.launch(Dispatchers.IO) {
        val now = System.currentTimeMillis()
        if (isCopy) {
            repeat(numberOfCopies) {
                ...
                for (plan in independentPlans) {
                    ...
                    syncTaskToCalendar(copy.copy(id = newId))
                    val checklists = dao.getChecklistSnapshot(plan.task.id)
                    for (chk in checklists) dao.insertChecklistItem(chk.copy(id = 0L, taskId = newId))
                    val attachments = dao.getAttachmentsSnapshot(plan.task.id)
                    for (att in attachments) dao.insertAttachment(att.copy(id = 0L, taskId = newId))
                }
            ...
            for (plan in independentPlans) {
                ...
                val siblings = dao.getSubtasksSnapshot(targetParentId)
                val updated = plan.task.copy(parentId = targetParentId, orderIndex = siblings.size, lastModifiedTimestamp = now)
                dao.updateTask(updated)
                syncTaskToCalendar(updated)
            }
